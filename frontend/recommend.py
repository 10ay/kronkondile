...
from frontend.feelings import FEELINGS_FILE, feelings_dictionary
from frontend.music_library import spotify_lovesick, spotify_mountains, spotify_silly, spotify_minions, spotify_love
...
def mood_to_book(feeling_index, rand_int = -1):
    """
    Maps your mood to a book dictionary.
    """
    if feeling_index not in feelings_dictionary:
        raise ValueError()
   # import pdb; pdb.set_trace()
    if rand_int != -1:
        ...
        rand_int = random.randint(0, 9)
    #import pdb; pdb.set_trace()
    return mood_book_map[feeling_index][rand_int]
...
def get_recommendations_for_mood_book(feeling_index, rand_int = -1):
    """
    Music recommendations for a given mood.
    """
    music_terms_to_search = mood_to_book(feeling_index, rand_int)
    track = open_book_from_dict(music_terms_to_search)
    return [track]

    return book_recommendation(music_terms_to_search)
...
def get_recommendations_book(rand_int = -1):
    """
    Get music recommendations for today.
    """
    feeling_index, feeling_name = get_feeling()
    if feeling_index is None:
        return []
    return get_recommendations_for_mood_book(feeling_index, rand_int)
